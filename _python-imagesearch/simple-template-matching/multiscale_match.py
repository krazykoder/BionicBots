...
def find_scaled_image (image, template):
...
def find_in_image (image, template, scale):
    # load the image, convert it to grayscale, and initialize the
    # bookkeeping variable to keep track of the matched region
    # image = cv2.imread(imagePath)
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
...
found = find_scaled_image(image, template)
...
template = cv2.imread ("assets/load-foup.bmp")
...
found = find_in_image(image, template, scale)
# found = find_scaled_image(image, template)
